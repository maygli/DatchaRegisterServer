...
func (server *AuthServer) shouldSendEmailConfirmation() bool {
...
func (server *AuthServer) SendConfirmationEmail(userId uint, userName string, email string, locale string) error {
	if server.SMTPServerURL == "" {
		return nil
	}
...
func (server *AuthServer) getMailTemplate(templFileName string, locale string) (string, error) {
	filePath := path.Join(TEMPLATE_FOLDER, templFileName+"_"+locale+TEMPLATE_EXTENSION)
	_, err := os.Stat(filePath)
	if err == nil {
		return filePath, nil
	}
	filePath = path.Join(TEMPLATE_FOLDER, templFileName+TEMPLATE_EXTENSION)
	_, err = os.Stat(filePath)
	return filePath, err
}

func (server *AuthServer) generateConfirmationMessage(userId uint, userName string, locale string) (bytes.Buffer, error) {
